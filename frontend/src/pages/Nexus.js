...
const Nexus = () => {
  ...
  const checkUploadProgress = async (fileId) => {
    try {
      const response = await axios.get(`https://nexus-orpin-five.vercel.app/upload_progress/${fileId}`);
  ...
  const handleFileUpload = async (event) => {
    ...
    if (fileExtension === 'csv') {
      ...
      try {
        const response = await axios.post('https://nexus-orpin-five.vercel.app/upload_to_github', formData, {
          headers: {
      ...
      try {
        const response = await axios.post('https://nexus-orpin-five.vercel.app/process_file', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
          onUploadProgress: (progressEvent) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      let response;
      if (fileType === 'csv') {
        response = await axios.post('https://nexus-orpin-five.vercel.app/get_results', {
          prompt: inputValue,
        ...
        response = await axios.post('https://nexus-orpin-five.vercel.app/query', {
          query: inputValue,
        });
